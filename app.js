var createError = require('http-errors');
var express = require('express');
var path = require('path');
var cookieParser = require('cookie-parser');
var logger = require('morgan');
var cors = require('cors');


// var indexRouter = require('./routes/index');
var indexRouter = require("./routes/indexRouter.js");
var ordersRouter = require("./routes/ordersRouter.js");
var supsRouter = require("./routes/supsRouter.js");
var authRouter = require("./routes/authRouter.js");
var profilesRouter = require("./routes/profilesRouter.js");
var { authByToken } = require("./controllers/authController")

var app = express();

process.env.VUE_APP_BASE_URL = 'http://api.spbsupboard.ru'

// view engine setup
// app.set('views', path.join(__dirname, 'views'));
// app.set('view engine', 'jade');

app.use(logger('dev'));
app.use(express.json());
app.use(express.urlencoded({ extended: false }));
app.use(cookieParser());
app.use(cors());
app.use(function (req, res, next) {
  // res.header("Access-Control-Allow-Origin", "http://localhost");
  res.header("Access-Control-Allow-Origin", "http://api.spbsupboard.ru");
  res.header("Access-Control-Allow-Origin", "*");
  res.header("Access-Control-Allow-Origin", "https://api.spbsupboard.ru");
  res.header("Access-Control-Allow-Origin", "http://spbsupboard.ru");
  res.header("Access-Control-Allow-Origin", "https://spbsupboard.ru");
  // res.header("Access-Control-Allow-Origin", process.env.VUE_APP_BASE_URL);
  res.header("Access-Control-Allow-Methods", "GET, PUT, POST");
  res.header("Access-Control-Allow-Headers", "Origin, X-Requested-With, Content-Type, Accept");
  next();
});
app.use(express.static(path.join(__dirname, '/front')));
app.use(express.static(path.join(__dirname, '/uploads')));

app.use(authByToken)

// app.use('/index', indexRouter);
app.use('/orders', ordersRouter);
app.use('/sups', supsRouter);
app.use('/auth', authRouter);
app.use('/profiles', profilesRouter);
app.use('/uploads', indexRouter);

// catch 404 and forward to error handler
app.use(function (req, res, next) {
  next(createError(404));
});

// error handler
app.use(function (err, req, res, next) {
  // set locals, only providing error in development
  res.locals.message = err.message;
  res.locals.error = req.app.get('env') === 'development' ? err : {};

  // render the error page
  res.status(err.status || 500);
  // res.render('error');
  console.log(err);
});

module.exports = app;
